...
The continuous function of f(x) must have at least two derivatives. As
usual $\beta$ is a parameter vector with $beta_j$ in its jth element
...
$$
## 1f
for a likelihood function depends on more than one parameters, the profile likelihood should be applied.
...
```{r}
## now maximize the likelihood pl
## which is similar to minimize -pl
#' @description function to calculate the maximum likelihood
#' @para xi the estimator vector (could have been calculated inside of the function)
#' @para lambda the smooth parameter
#' @para sigma the standard deviance from the sample if known
#' @para X the original independent variables
#' @para y the outcome vector
#' @para k number of knots
#' @example see below the function
get_maxpl <-
  function(xi, lambda, sigma, X, y, k = 25) {
    sm_cr <- mgcv::smoothCon(
        s(x, ## a list of variables as covariates
          ## this smooth is a function of
          bs = "cr",
          ## bs for penalized smoothing basis
    ...
    ## obtain \hat \xi
    ## Fri Mar 26 21:30:35 2021 ------------------------------
    ## update the penalty term as 1/2
    solution <- solve(crossprod(phi) + lambda * sigma^2 * s_matrix)
    ## compute log profile likelihood...
    pl <- -(n * log(2 * pi) + log(norm(V, type = "O")) +
      t(y - phi %*% xi) %*% solve(V) %*% (y - phi %*% xi)) / 2 -
      0.5 * lambda * t(xi) %*% s_matrix %*% xi
    
    # Sat Mar 13 16:45:50 2021 ------------------------------
    ## check for xi_hat value
    xi_hat <- solution %*% t(phi) %*% y
    colnames(xi_hat) <- "xi_hat" 
    # print(xi_hat)
    attr(pl, "xi_hat") <- xi_hat
    xi_se <- 
      solve(t(phi) %*% solve(V) %*% phi +
              lambda * s_matrix) %>%
      diag() %>% 
      sqrt()
    attr(pl, "xi_se") <- xi_se
...
## 4c penalized mean error square
